...
import { SCRATCH_DIR, ASSETS_DIR, THUMB_DIR, OG_DIR, RECEIPTS_PATH } from "../shared/paths.js";
...
async function main() {
  ...
  let entries: any[] = readFileSync(songsPath, "utf-8")
    .trim()
    .split("\n")
    .map((line) => JSON.parse(line))
    .filter((e: any) => !e.song.internalImageId);
  ...
  console.log(`\nDone! ${receipts.length} receipts written to ${RECEIPTS_PATH}`);
}
